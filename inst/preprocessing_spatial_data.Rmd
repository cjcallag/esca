...
colnames(esca_parcels) <- sub("FortOrd.DBO.tblParcel.", "", colnames(esca_parcels))
colnames(esca_parcels) <- sub("FortOrd.DBO.PARCEL_AREA.", "", colnames(esca_parcels))
esca_parcels <- esca_parcels[, c("COENumber", "parcel_id", "ParcelName", "HMP_category",
                                 "Authority", "Category", "Acreage", "AcreageType",
                                 "OESite", "FOSETDoc", "FOSETStatus", "FOSETDate",
                                 "TrackingNumber_Name", "NewFORAName", "EnvironmentalComments",
                                 "BRACComments", "TransferDocName", "imparea_id",
                                 "MRA", "Jurisdiction")]
...
```{r}
lucs <- sf::st_read("https://services2.arcgis.com/nOGTdfb4kF4dZljH/ArcGIS/rest/services/FtOrd_LUCs/FeatureServer/0/query?where=1%3D1&outFields=*&outSR=4326&f=json")
lucs <- lucs[lucs$COENumber %in% esca_parcels[["COENumber"]], ] %>%
  st_set_geometry(NULL)
esca_parcels <- merge(x = esca_parcels,
                      y = lucs[, c("COENumber", "Jurisdicti",
                                   "Covenant", "C1", "C2", "C3", "C4", "C5",
                                   "C6", "C7", "C8", "C9")],
                      by = "COENumber", all.x = TRUE)
esca_parcels$Jurisdicti <- ifelse(test = is.na(esca_parcels$Jurisdicti),
                                  esca_parcels$Jurisdiction,
                                  esca_parcels$Jurisdicti)
```
...
All of these came from field visits and stored as a geojson:

```{r}
signs <- sf::read_sf(here::here("inst/extdata/Labeled Places.json")) %>%
  filter(name == "esca_sign") %>%
  select(name) %>%
  st_intersection(., ft_ord)
```

```{r}
ggplot() +
  geom_sf(data = ft_ord, fill = "white", size = 0.5) +
  geom_sf(data = esca, color = NA, fill = "slategrey") +
   geom_sf(data = signs, size = 0.5) +
  theme_minimal()
```
...
```{r}
save(signs, file = here::here("data/signs.rda"))
```
